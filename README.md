...
Создать модуль, реализующий хранение в одном файле данных в виде графа узлов с атрибутами общим объёмом от 10GB, имеющий вид реляционной БД
...
2. Файл БД логически поделен на страницы одинакового размера. При создании новой страницы, в ее начало записывается заголовок страницы: ее номер, информация о ее заполненности, информация о таблице, которой она принадлежит или о БД в случае служебной страницы. Каждая страница принадлежит только одной таблице.
3. Первая страница в файле БД - служебная. При создании БД в нее записываются заголовок БД, заголовок служебной таблицы.
4. Для создания таблицы предварительно нужно создать схему таблицы. Схема таблицы содержит в себе описания колонок: их названия, тип данных, размер в байтах. Также в схеме хранится длина строки в байтах.
5. При создании таблицы идет ее запись в файл БД. Во-первых, создается заголовок таблицы и записывается в служебную страницу. Заголовок таблицы содержит информацию о названии таблицы, номерах ее первой и последней страницы, валидности таблицы. Во-вторых, создается первая страница таблицы. В эту страницу записывается заголовок страницы (его величина одинакова фдля всех таблиц), длина в байтах информации о колонках таблицы, а дальше идут данные переменной длины: информация о колонках, из которых состоит схема. Благодаря хранению длины информации о колонках, мы при считывании таблицы с БД знаем, сколько байт надо считать, чтобы прочитать информацию о колонках, т.к. эта длина для таблиц разная. 
...
```
6. Все остальное место в странице оставлено для заголовков строк и самих строк. 
...
10. Для доступа к данным нужно создавать и запускать запросы. В зависимости от типа запроса будет выбрана соответствующая логика. Запросы получают на вход таблиц_ы, названи_я колонок, значени_я переменных, котор_ые использу_ются в запросе. По названию колонки будет определен тип переменной и выбрана соответствующая логика.
